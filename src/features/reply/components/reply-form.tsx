...
export function ReplyForm({ recipientName, recipientEmail }: Props) {
  ...
  function generateMessage() {
    ...
    if (!person.trim()) return setError("担当者名を入力してください。");
    if (!email.trim()) return setError("返信用メールアドレスを入力してください。");
  ...
  return (
    <div className="grid gap-10 py-12 lg:grid-cols-[.38fr_.62fr] lg:gap-16">
      <aside className="lg:sticky lg:top-10 lg:self-start"><p className="eyebrow mb-5">How it works</p><ol className="space-y-5">{["連絡先と選考結果を入力", "作成した文面を確認", "この画面、またはメールソフトから送信"].map((item, index) => <li key={item} className="flex gap-4 border-t border-(--color-border) pt-4 text-sm leading-6"><span className="font-mono text-(--color-accent)">0{index + 1}</span><span>{item}</span></li>)}</ol></aside>
      <section aria-label="選考結果連絡フォーム" className="space-y-10">
        <fieldset><legend className="mb-5 text-xl font-semibold tracking-tight">ご担当者情報</legend><div className="grid gap-5 sm:grid-cols-2"><div><label htmlFor="company" className={labelClass}>会社名</label><input id="company" value={company} onChange={e => setCompany(e.target.value)} className={fieldClass} autoComplete="organization" /></div><div><label htmlFor="person" className={labelClass}>担当者名</label><input id="person" value={person} onChange={e => setPerson(e.target.value)} className={fieldClass} autoComplete="name" /></div><div className="sm:col-span-2"><label htmlFor="email" className={labelClass}>返信用メールアドレス</label><input id="email" type="email" value={email} onChange={e => setEmail(e.target.value)} className={fieldClass} autoComplete="email" /></div></div></fieldset>
        ...
        <button type="button" onClick={generateMessage} className="interactive w-full cursor-pointer rounded-(--radius-m) bg-(--color-surface-strong) px-5 py-4 font-semibold text-white shadow-(--shadow-m) hover:bg-(--color-accent)">連絡文を作成する</button>
