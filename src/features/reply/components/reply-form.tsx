...
const fieldClass = "mt-2 w-full rounded-m border border-border bg-surface px-4 py-3.5 outline-none transition-colors placeholder:text-muted/55 focus:border-accent";
  ...
  return (
    <div className="grid gap-10 py-12 lg:grid-cols-[.38fr_.62fr] lg:gap-16">
      ...
      <section aria-label="選考結果連絡フォーム" className="space-y-10">
        ...
        <fieldset><legend className="mb-5 text-xl font-semibold tracking-tight">選考結果</legend><div className="grid gap-3 sm:grid-cols-2">{[["passed", "書類通過・面談日程調整"], ["rejected", "今回は見送り"]].map(([value, label]) => <label key={value} className={`interactive cursor-pointer rounded-m border p-4 ${selectionResult === value ? "border-accent bg-accent-soft" : "border-border bg-surface"}`}><input type="radio" name="selection_result" value={value} checked={selectionResult === value} onChange={() => setSelectionResult(value as SelectionResult)} className="mr-3 accent-accent" />{label}</label>)}</div></fieldset>
        {selectionResult === "passed" && <fieldset className="space-y-5 border-l-2 border-accent pl-5"><legend className="mb-5 text-xl font-semibold">面談について</legend><div><label htmlFor="interview_dates" className={labelClass}>面談候補日</label><textarea id="interview_dates" rows={4} value={interviewDates} onChange={e => setInterviewDates(e.target.value)} placeholder="例：9月18日（金）10:00〜12:00" className={fieldClass} /></div><div><label htmlFor="passed_note" className={labelClass}>補足メッセージ</label><textarea id="passed_note" rows={4} value={passedNote} onChange={e => setPassedNote(e.target.value)} className={fieldClass} /></div></fieldset>}
        {selectionResult === "rejected" && <fieldset className="space-y-5 border-l-2 border-accent pl-5"><legend className="mb-5 text-xl font-semibold">フィードバック</legend><div><label htmlFor="rejection_reason" className={labelClass}>見送り理由 <span className="text-danger">必須</span></label><textarea id="rejection_reason" rows={5} value={rejectionReason} onChange={e => setRejectionReason(e.target.value)} className={fieldClass} /></div><div><label htmlFor="improvement_points" className={labelClass}>改善するとよい点</label><textarea id="improvement_points" rows={5} value={improvementPoints} onChange={e => setImprovementPoints(e.target.value)} className={fieldClass} /></div></fieldset>}
        <button type="button" onClick={generateMessage} className="interactive w-full cursor-pointer rounded-m bg-surface-strong px-5 py-4 font-semibold text-white shadow-m hover:bg-accent">入力内容から連絡文を作成</button>
        <div aria-live="polite">{error && <p role="alert" className="border-l-2 border-danger pl-4 text-sm font-medium text-danger">{error}</p>}{notice && <p className="border-l-2 border-success pl-4 text-sm font-medium text-success">{notice}</p>}</div>
        {previewMessage && <section className="rounded-l bg-surface p-5 shadow-m sm:p-8"><div className="mb-5 flex items-center justify-between gap-4"><h2 className="text-xl font-semibold">送信内容の確認</h2><button type="button" onClick={copyMessage} className="interactive text-sm font-semibold text-accent">コピー</button></div><pre className="whitespace-pre-wrap font-sans text-sm leading-7 text-muted">{previewMessage}</pre><div className="mt-8 grid gap-3 sm:grid-cols-2"><button type="button" onClick={sendMessage} disabled={isSending || isSent} className="interactive cursor-pointer rounded-m bg-accent px-5 py-3.5 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-55">{isSent ? "送信済み" : isSending ? "送信中…" : "この内容で送信"}</button><button type="button" onClick={openMailClient} className="interactive cursor-pointer rounded-m border border-border px-5 py-3.5 font-semibold hover:border-accent">メールソフトで送る</button></div></section>}
      </section>
    </div>
  );
}
